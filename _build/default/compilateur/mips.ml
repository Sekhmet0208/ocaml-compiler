type reg = Zero | SP | RA | FP | V0 | A0 | A1 | T0 | T1 | T2
type label = string
...
let fmt_loc = function Lbl l -> l | Mem (r, o) -> ps "%d(%s)" o (fmt_reg r)
...
let fmt_instr = function
  ...
  | Add (rd, rs, rt) ->
      ps "  add %s, %s, %s" (fmt_reg rd) (fmt_reg rs) (fmt_reg rt)
  | Sub (rd, rs, rt) ->
      ps "  sub %s, %s, %s" (fmt_reg rd) (fmt_reg rs) (fmt_reg rt)
  | Div (rd, rs, rt) ->
      ps "  div %s, %s, %s" (fmt_reg rd) (fmt_reg rs) (fmt_reg rt)
  | Mul (rd, rs, rt) ->
      ps "  mul %s, %s, %s" (fmt_reg rd) (fmt_reg rs) (fmt_reg rt)
  | Syscall -> ps "  syscall"
  | B l -> ps "  b %s" l
  ...
  | Slt (rd, rs, rt) ->
      ps "  slt %s, %s, %s" (fmt_reg rd) (fmt_reg rs) (fmt_reg rt)
